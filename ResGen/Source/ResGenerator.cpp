...
ResGenerator::ResGenerator(const ToolCtx& toolCtx, SymTable&& symTable) : m_SymTable(std::move(symTable)) {
    m_ResolverName = toolCtx.resolverName();
}

void ResGenerator::writeToFile(const std::filesystem::path& path) {
    std::ofstream f(path);
    if (!f.is_open()) {
        resgen::printError({}, "could not open file \"{}\" for output", path.string());
        return;
    }
